...
public class FizikFormulleri {
    ...
    public static void main(String[] args) {
        ...
        System.out.printf("Velocity: %.2f", calculateVelocity(mass, time));
        double velocity = calculateVelocity(mass, time);
        System.out.printf("Acceleration: %.2f", calculateAcceleration(deltaV, time));
        double acceleration = calculateAcceleration(deltaV, time);
        System.out.printf("Force: %.2f", calculateForce(mass, deltaV));
        double force = calculateForce(mass, deltaV);
        System.out.printf("Work: %.2f", calculateWork(mass, height));
        double work = calculateWork(mass, height);
        System.out.printf("Power: %.2f", calculatePower(force, work));
        double power = calculatePower(force, work);
        System.out.printf("Kinetic Energy: %.2f", calculateKineticEnergy(mass, velocity));
        double kineticEnergy = calculateKineticEnergy(mass, velocity);
        System.out.printf("Potential Energy: %.2f" , calculatePotentialEnergy(mass, GRAVITY , height));
        double potentialEnergy  = calculatePotentialEnergy(mass, GRAVITY , height);
        System.out.printf("Momentum: %.2f", calculateMomentum(mass, velocity));
